...
`docker compose up` gives you a working product: `pos-db` creates the
...
By default these talk to the `pos-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/pos npm run demo:status
```
