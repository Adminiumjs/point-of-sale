...
const COLS: { k: KdsStatus; l: MessageKey; ic: string; col: string }[] = [
  { k: 'new', l: 'kitchen.colNew', ic: 'inbox', col: 'var(--accent)' },
  { k: 'cooking', l: 'kitchen.colCooking', ic: 'flame', col: 'var(--warn)' },
  { k: 'ready', l: 'kitchen.colReady', ic: 'check-check', col: 'var(--pos)' },
];
  ...
  return (
    <div style={css('flex:1;min-height:0;display:flex;flex-direction:column;background:var(--surface-2);')}>
      ...
      <div className="pos-scroll" style={css('flex-shrink:0;display:flex;align-items:center;gap:9px;padding:11px 16px;overflow-x:auto;background:var(--surface);border-bottom:1px solid var(--border);')}>
        <span style={css('font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-subtle);flex-shrink:0;')}>{t('kitchen.allDay')}</span>
        {allDay.map((a) => (
          ...
          return (
            <div key={c.k} className="kds-col">
              <div style={css('flex-shrink:0;display:flex;align-items:center;gap:9px;padding:14px 16px;border-bottom:1px solid var(--border);')}>
                ...
                <span style={css('font-size:15px;font-weight:800;')}>{t(c.l)}</span>
                <span style={css('margin-inline-start:auto;font-size:12.5px;font-weight:800;' + MONO + 'padding:2px 9px;border-radius:20px;background:var(--surface-3);color:var(--fg-muted);')}>{cards.length}</span>
              </div>
                  ...
                  return (
                    <div key={o.number} style={css('background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px;box-shadow:var(--shadow);')}>
                      <div style={css('display:flex;align-items:center;gap:8px;margin-bottom:11px;')}>
                        ...
                        <span style={css('display:inline-flex;align-items:center;gap:4px;margin-inline-start:auto;font-size:11.5px;font-weight:800;padding:3px 9px;border-radius:20px;background:' + (urgent ? 'var(--danger-soft)' : 'var(--surface-3)') + ';color:' + (urgent ? 'var(--danger)' : 'var(--fg-muted)') + ';')}>
                          <Icon name="clock" size={12} />
                          {t('common.minutesShort', { m: age })}
                        </span>
                      </div>
                      ...
                      <button className="pos-press" onClick={() => s.bumpK(o.number)} style={css('width:100%;height:48px;border-radius:12px;border:none;background:' + (c.k === 'ready' ? 'var(--pos)' : 'var(--accent)') + ';color:' + (c.k === 'ready' ? 'var(--pos-fg)' : 'var(--accent-fg)') + ';font-size:15px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;')}>
                        ...
                        {t(c.k === 'ready' ? 'kitchen.clearTicket' : 'kitchen.bump')}
                      </button>
                    </div>
                  );
                })}
